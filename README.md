<!-- autogenerated using @gitlab.com/konradodwrot/cross-repo/prose/assets//repos/cross-repo/infra/iac/templates/3-audience/README.md.ontoRepo.tpl?ref=v0.0.55 -->
# infra
...
Terraform for the `konradodwrot` GitLab group: group tree, every project, branch protection, GitHub mirroring, and the machine identities automation runs on. The identities: sandbox (Developer group token, SSH access and signing keys, a least-privileged GCP service account), automation (Maintainer group token for regen MRs, a masked CI variable on the automation project only), the prose tag-minting project token, the homebrew-tap publisher token, release signing. Secrets flow through GCP Secrets Manager and 1Password. State sits in its own GCS bucket.
...
- Automation identities as code: sandbox (Developer: write, no delete), automation (Maintainer: regen MRs, auto-merge), prose tagger, release signing.
...
`glab variable get <KEY> -R konradodwrot/cross-repo/infra/iac`.
